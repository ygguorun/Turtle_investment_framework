...
class FinancialsMixin:
    ...
    def get_basic_info(self, ts_code: str) -> str:
        ...
        if not daily.empty:
            ...
            val_rows = [
                ...
                [
                    "总市值 (万元)",
                    format_number(d.get("total_mv", None), divider=1, decimals=2),
                ],
                [
                    "流通市值 (万元)",
                    format_number(d.get("circ_mv", None), divider=1, decimals=2),
                ],
            ]
        else:
            yf_px = self._yf_fallback_price(ts_code)
            if yf_px and yf_px.get("close"):
                close = self._safe_float(yf_px.get("close"))
                market_cap = self._safe_float(yf_px.get("market_cap"))
                total_mv_wan = (market_cap / 10000.0) if market_cap else None
                total_share_wan = (
                    (market_cap / close / 10000.0)
                    if (market_cap and close and close > 0)
                    else None
                )
                self._store["basic_info"] = pd.DataFrame(
                    [
                        {
                            "ts_code": ts_code,
                            ...
                            "float_share": total_share_wan,
                        }
                    ]
                )
                val_rows = [
                    ["当前价格", f"{close:.2f}" if close else "—"],
                    ["PE (TTM)", "—"],
                    ["PB", "—"],
                    [
                        "总市值 (万元)",
                        format_number(total_mv_wan, divider=1, decimals=2),
                    ],
                    ["流通市值 (万元)", "—"],
                ]
            ...
            if not fina.empty:
                ...
                val_rows = [
                    ...
                    [
                        "总市值 (百万港元)",
                        format_number(
                            d.get("total_market_cap", None), divider=1, decimals=2
                        ),
                    ],
                ]
        except RuntimeError:
    ...
    def _get_basic_info_us(self, ts_code: str) -> str:
        ...
        basic = self._cached_basic_call(
            "us_basic", ts_code=api_code, fields="ts_code,name,enname,market,list_date"
        )
        if basic.empty:
            return format_header(2, "1. 基本信息") + "\n\n数据缺失\n"
        ...
        try:
            ...
            if not daily.empty:
                ...
                val_rows = [
                    ...
                    [
                        "总市值 (百万美元)",
                        format_number(d.get("total_mv", None), divider=1e6, decimals=2),
                    ],
                ]
        except RuntimeError:
            pass
    ...
    def get_market_data(self, ts_code: str) -> str:
        ...
        if df.empty:
            try:
                ...
                yf_daily = ticker.history(period="1y", interval="1d")
            except Exception:
                yf_daily = pd.DataFrame()
            if yf_daily.empty:
                lines.append("数据缺失\n")
                return "\n".join(lines)
            yf_daily = yf_daily.reset_index().rename(
                columns={
                    "Date": "trade_date",
                    "Open": "open",
                    "High": "high",
            ...
            yf_daily["trade_date"] = pd.to_datetime(yf_daily["trade_date"]).dt.strftime(
                "%Y%m%d"
            )
            df = yf_daily[["trade_date", "open", "high", "low", "close", "vol"]].copy()
            lines.append("*来源: yfinance (降级)*\n")
    ...
    def _get_market_data_hk(self, ts_code: str) -> str:
        ...
        if yf_data and yf_data.get("close"):
            ...
            if yf_data.get("market_cap"):
                rows.append(
                    ["总市值", format_number(yf_data["market_cap"], divider=1e6)]
                )
        ...
        if not df.empty:
            ...
            table = format_table(
                ...
                [
                    ...
                    [
                        "52周涨跌幅",
                        f"{(latest_close / low_52w - 1) * 100:.1f}% (自低点)",
                    ],
    ...
    def _get_income_hk(self, ts_code: str) -> str:
        ...
        df = self._safe_call(
            "hk_income", ts_code=ts_code, fields="ts_code,end_date,ind_name,ind_value"
        )
    ...
    def _get_income_us(self, ts_code: str) -> str:
        ...
        api_code = self._us_api_code(ts_code)
        df = self._safe_call(
            "us_income", ts_code=api_code, fields="ts_code,end_date,ind_name,ind_value"
        )
        lines = [format_header(2, "3. 合并利润表"), ""]
    ...
    def get_balance_sheet(self, ts_code: str, report_type: str = "1") -> str:
        ...
        section_label = (
            "4P. 母公司资产负债表" if report_type == "6" else "4. 合并资产负债表"
        )
        lines = [format_header(2, section_label), ""]
    ...
    def get_cashflow(self, ts_code: str) -> str:
        ...
        for _, r in df.iterrows():
            ...
            vals = [
                v
                for v in [depr, amort_intang, amort_deferred]
    ...
    def _get_cashflow_hk(self, ts_code: str) -> str:
        ...
        df = self._safe_call(
            "hk_cashflow", ts_code=ts_code, fields="ts_code,end_date,ind_name,ind_value"
        )
        lines = [format_header(2, "5. 现金流量表"), ""]
    ...
    def get_dividends(self, ts_code: str) -> str:
        ...
        for _, r in df.iterrows():
            ...
            total_div = (
                cash_div * base_share * 10000
            )  # base_share is 万股, convert to shares
            rows.append(
                [
                    year,
                    f"{cash_div:.4f}",
        ...
        table = format_table(headers, rows, alignments=["l", "r", "r", "l", "l", "r"])
        lines.append(table)
        return "\n".join(lines)
    ...
    def _get_dividends_hk(self, ts_code: str) -> str:
        ...
        dps_values = [
            self._safe_float(r.get("dps_hkd"))
            for _, r in df.iterrows()
            if self._safe_float(r.get("dps_hkd")) is not None
            and self._safe_float(r.get("dps_hkd")) > 0
        ]
        dps_looks_stuck = (
            len(dps_values) >= 3 and len(set(round(v, 4) for v in dps_values)) == 1
        )
    ...
    def _get_dividends_us(self, ts_code: str) -> str:
        ...
        headers = ["年度", "每股股息 (USD)"]
        rows = [
            [str(int(r["year"])), f"{r['dividend']:.4f}"] for _, r in annual.iterrows()
        ]
    ...
    def get_weekly_prices(self, ts_code: str) -> str:
        ...
        today = pd.Timestamp.now().strftime("%Y%m%d")
        ten_years_ago = (pd.Timestamp.now() - pd.DateOffset(years=10)).strftime(
            "%Y%m%d"
        )
        ...
        if df.empty:
            try:
                ...
                if not daily.empty:
                    ...
                    weekly = (
                        daily.resample("W-FRI", on="trade_date")
                        .agg(
                            {
                                "open": "first",
                                "high": "max",
                    ...
                    if bs is not None and not bs.empty:
                        ...
                        if not eq_row.empty:
                            equity = self._safe_float(
                                eq_row.iloc[0].get("total_hldr_eqy_exc_min_int")
                            )
                ...
                if pd.isna(row.get("holder_profit_yoy")):
                    ...
                    if inc is not None and not inc.empty:
                        ...
                        if not curr.empty and not prior_dates.empty:
                            ...
                            prior_np = self._safe_float(
                                prior_dates.iloc[0].get("n_income")
                            )
                            if curr_np is not None and prior_np and prior_np != 0:
                                df.at[idx, "holder_profit_yoy"] = (
                                    (curr_np - prior_np) / abs(prior_np) * 100
                                )
