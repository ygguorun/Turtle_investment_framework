...
**补充关注点**（非独立子项，视数据可得性酌情展开）：

- **有息负债结构**：若 §4 显示 st_borr + lt_borr + bond_payable 规模较大（有息负债/总资产 > 20%），评估利息保障能力（EBITDA / finance_exp > 3x？）、现金覆盖率（money_cap / 有息负债 > 1.0？）、融资成本水平。区分经营杠杆（固定成本结构，可接受）与财务杠杆（有息借款，需警惕）
- **利润来源拆分**：若 §3 invest_income 占利润总额 > 20%，标注"投资收益依赖型"，评估可持续性和现金回流（§5 c_recp_return_invest）。若长期股权投资占净资产比重大且以公允价值计量，标注 PB 可能失真
